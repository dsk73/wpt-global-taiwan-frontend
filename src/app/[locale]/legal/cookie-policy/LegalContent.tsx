...
/**
 * ReactMarkdown treats lines such as:
 *
 * 1. What are cookies?
 * 2. Use of cookie files
 * 3. How to manage the use of cookies
 *
 * as ordered-list items.
 *
 * We escape the "." after the number so ReactMarkdown
 * renders the line as a normal paragraph.
 *
 * This allows us to control the number and styling ourselves.
 */
...
function normalizeMarkdown(content: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (structural) {
       ...
       * 3. How to manage...
       *
       * from becoming Markdown ordered lists.
       *
       * We escape the "." so ReactMarkdown keeps them as
       * normal paragraphs.
       * ========================================================
       */
...
type CookieLevel = 1 | 2 | 3 | 4 | 5 | null;

function getCookieLevel(text: string): CookieLevel {
...
/* ============================================================
   Extract Cookie Number
============================================================ */

function getCookieNumber(text: string): string | null {
  const value = text.trim();
...
/* ============================================================
   Cookie Paragraph Styling
============================================================ */

function CookieParagraph({ children, fullContent }: CookieParagraphProps) {
  const text = extractText(children);

  const level = getCookieLevel(text);

  const cookieNumber = getCookieNumber(text);

  const levelTwoHasSubpoints =
    level === 2 && cookieNumber
      ? hasSubpoints(cookieNumber, fullContent)
      : false;
   ...
   * ALWAYS BOLD
   *
   * These are the main Cookie Policy sections.
   * ==========================================================
   */
              ...
              <ReactMarkdown
                ...
                components={{
                  ...
                  p: ({ children }) => {
                    ...
                    /*
                     * Numbered Cookie Policy sections
                     */

                    if (level !== null) {
                      return (
                        <CookieParagraph fullContent={normalizedContent}>
                          {children}
                        </CookieParagraph>
                      );
                    }
