...
const acceptanceContent = {
  en: getModuleContent(acceptanceEnModule as Record<string, unknown>),

  zh: getModuleContent(acceptanceZhModule as Record<string, unknown>),

  ms: getModuleContent(acceptanceMsModule as Record<string, unknown>),
};
...
export default async function CustomerAcceptancePolicyPage({
  params,
}: PageProps) {
  const { locale } = await params;
  ...
  const content = acceptanceContent[language];
  ...
  if (!content) {
    console.error(
      `Customer Acceptance Policy content is missing for locale: ${locale}`,
    );
  }
