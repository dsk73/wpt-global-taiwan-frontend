// src/app/[locale]/legal/kyc-policy/LegalContent.tsx
...
/* ============================================================
   Escape Top-Level Numbered Sections
============================================================ */

/*
 * ReactMarkdown normally interprets:
 *
 * 1. For proof of identity...
 * 2. For proof of address...
 * 3. For proof of deposit...
 *
 * as an ordered list.
 *
 * We escape the period after the number so that the number
 * remains part of the paragraph and can be styled by us.
 */

function escapeNumberedSection(line: string): string {
...
function normalizeMarkdown(content: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (structural) {
      ...
      /*
       * Keep numbered KYC sections as paragraphs instead of
       * allowing ReactMarkdown to turn them into <ol>.
       */

      if (/^\d+\.\s+/.test(trimmed)) {
        output.push(escapeNumberedSection(line));
      } else {
        output.push(line);
      }
...
type KycLevel = 1 | 2 | 3 | 4 | 5 | null;

function getKycLevel(text: string): KycLevel {
...
function getKycNumber(text: string): string | null {
  const value = text.trim();
...
/* ============================================================
   Check Whether Level-2 Section Has Subpoints
============================================================ */
...
interface KycParagraphProps {
  children?: ReactNode;
  fullContent: string;
}

function KycParagraph({ children, fullContent }: KycParagraphProps) {
  const text = extractText(children);

  const level = getKycLevel(text);

  const kycNumber = getKycNumber(text);

  const levelTwoHasSubpoints =
    level === 2 && kycNumber ? hasSubpoints(kycNumber, fullContent) : false;
  ...
  return (
    <main className="bg-[#050505] text-white">
      {/* ======================================================
          ...
          Back / Legal navigation, page title and Last Updated
          are intentionally handled by page.tsx.
      ====================================================== */}
              ...
              <ReactMarkdown
                ...
                components={{
                  ...
                  p: ({ children }) => {
                    ...
                    /*
                     * Numbered KYC content
                     */

                    if (level !== null) {
                      return (
                        <KycParagraph fullContent={normalizedContent}>
                          {children}
                        </KycParagraph>
                      );
                    }
